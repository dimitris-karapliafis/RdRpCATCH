...
def parse_args():
    ...
    ## Scanning options
    scanner_parser = subparsers.add_parser('scan', help='Scan sequences for RdRps.')
    scanner_group = scanner_parser.add_argument_group('Scanner options')
    ...
    scanner_group.add_argument("-dbs", "--db_options", type=parse_comma_separated_options,
                               ...
                               help="Comma-separated list of databases to search against. Choose from: RVMT,"
                                    ...
                                    " Lucaprot,"
                                    " all")
    scanner_group.add_argument("-hmm_dir", "--hmm_dir", help="Path to the directory containing HMM databases.",
                               default=None)
    ...
    ## Scanning options -- HMMsearch
    hmmsearch_group = scanner_parser.add_argument_group('HMMsearch options')
    hmmsearch_group.add_argument('-e', '--evalue', type=float, default=1e-5, help="E-value threshold for HMMsearch.")
    hmmsearch_group.add_argument('-incE', '--incEvalue', type=float, default=1e-5,
                                 help="Inclusion E-value threshold for HMMsearch.")
    hmmsearch_group.add_argument('-domE', '--domEvalue', type=float, default=1e-5,
                                 help="Domain E-value threshold for HMMsearch.")
    hmmsearch_group.add_argument('-incdomE', '--incdomEvalue', type=float, default=1e-5,
                                 help="Inclusion domain E-value threshold for HMMsearch.")
    ...
    ## Scanning options -- Seqkit seq
    seqkit_seq_group = scanner_parser.add_argument_group('Seqkit seq options')
    seqkit_seq_group.add_argument('-length_thr', '--length_thr', type=int, default=400,
                                  help="Minimum length threshold for seqkit seq.")
    ## Scanning options -- Seqkit translate
    seqkit_transl_group = scanner_parser.add_argument_group('Seqkit translate options')
    seqkit_transl_group.add_argument('-gen_code', '--gen_code', type=int, default=1,
    ...
    ## Download options
    download_parser = subparsers.add_parser('download', help='Download HMM databases.')

    download_group = download_parser.add_argument_group('Database download options')
    download_group.add_argument('-dest', '--destination_dir', help="Path to the directory to download HMM databases.", default=None)
    ## GUI options
